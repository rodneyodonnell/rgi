...
class AlphaZeroPlayer(Player[TGameState, TPlayerState, TAction], Generic[TGame, TGameState, TAction]):
    def __init__(self, game: TGame, network: PolicyValueNetwork[TGame, TGameState, TAction]) -> None:
        self.game = game
    ...
    def select_action(self, game_state: TGameState, legal_actions: Sequence[TAction]) -> TAction:
        ...
        action_visits = mcts.search(game_state, legal_actions)
...
class MCTSNode(Generic[TGame, TGameState, TAction]):
    def __init__(
        self, state: TGameState, n_players: int, parent: Optional["MCTSNode[TGame, TGameState, TAction]"] = None
    ) -> None:
