...
def test_legal_actions_initial(game):
    state = game.initial_state()
...
def test_next_state_simple_move(game):
    state = game.initial_state()
...
def test_reward_win(game):
    # Create a winning state for player 1
    board = Map({(row, col): 1 for row in range(1, game.board_size + 1) for col in range(1, game.board_size + 1)})
    state = OthelloState(board=board, current_player=1, is_terminal=True)
    assert game.reward(state, 1) == 1.0
    assert game.reward(state, 2) == -1.0


def test_reward_draw(game):
    # Create a draw state
    board = Map(
        {
...
def test_full_game_simulation(game):
...
def test_full_board_playthrough(game):
...
def test_flipping_multiple_directions(game):
...
def test_no_legal_moves_for_current_player(game):
...
def test_game_end_by_no_moves(game):
    board_str = """
    . . . . . . . .
    . . . . . . . .
    . . . . . . . .
...
def test_parse_board(game, board_str, positions):
    state = game.parse_board(board_str, current_player=1, is_terminal=False)
...
def test_pretty_str_bottom_left(game):
