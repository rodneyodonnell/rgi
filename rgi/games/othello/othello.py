...
class OthelloGame(base.Game[GameState, Action]):
...
class OthelloSerializer(base.GameSerializer[OthelloGame, OthelloState, Action]):
    @override
    def serialize_state(self, game: OthelloGame, game_state: OthelloState) -> dict[str, Any]:
        return {
            "rows": game_state.board.shape[0],
            "columns": game_state.board.shape[1],
            "board": game_state.board.tolist(),
            "current_player": game_state.current_player,
            "legal_actions": game.legal_actions(game_state),
